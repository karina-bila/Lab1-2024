...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->button2->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 15, System::Drawing::FontStyle::Regular, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(204)));
			this->button2->Location = System::Drawing::Point(364, 467);
			...
			this->label3->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 10, System::Drawing::FontStyle::Regular, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(204)));
			this->label3->Location = System::Drawing::Point(12, 529);
			...
			this->label1->Text = L"Введіть масив через пробіл";
			// 
			// label2
			// 
			...
			this->label2->Size = System::Drawing::Size(522, 39);
			this->label2->TabIndex = 2;
			this->label2->Text = L"Обробка одновимірного масиву";
			// 
			// label4
			// 
			...
			this->textBox2->Size = System::Drawing::Size(479, 234);
			...
			this->ClientSize = System::Drawing::Size(544, 555);
	...
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		//перевірка чи введено масив
		if (textBox1->Text == "") {
			MessageBox::Show("Введіть масив через пробіл", "Помилка", MessageBoxButtons::OK, MessageBoxIcon::Error);
			textBox1->Focus();
			return;
		}

		//розділюємо введений рядок на масив чисел
		array<String^>^ arr = textBox1->Text->Split(' ');

		//рахуємо середнє арифметичне додатних елементів масиву
		double sum = 0;
		int count = 0;
		for (int i = 0; i < arr->Length; i++) {
			double num;
			if (Double::TryParse(arr[i], num)) {
				if (num > 0) {
					sum += num;
					count++;
				}
		...
		//сортування масиву методом екстремальних значень
		for (int i = 0; i < arr->Length / 2; i++) {
			double min = Double::Parse(arr[i]);
			double max = Double::Parse(arr[i]);
			int minIndex = i;
			int maxIndex = i;
		...
		textBox2->Text = "Середнє арифметичне додатних елементів масиву: " + result + "\r\n";
		textBox2->Text += "Масив після сортування: ";
		for (int i = 0; i < arr->Length; i++) {
			textBox2->Text += arr[i] + " ";
		}
